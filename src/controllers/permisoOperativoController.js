/**
 * Controlador PTRI — Permiso de Trabajo con Riesgo de Incendio
 *
 * Flujo y roles:
 *   SOLICITANTE/ADMIN → crear (PENDIENTE)
 *   BOMBERO/ADMIN     → evaluar · implantar · cerrar
 *   JEFE/ADMIN        → autorizar · rechazar
 *   JEFE/ADMIN/owner  → anular
 */
...
const { enviarNotificacion, tokensPorRol, tokenDeUsuario } = require('../utils/notifications');
...
exports.listar = asyncHandler(async (req, res) => {
  ...
  if (rol === 'SOLICITANTE') {
    ...
    if (!todos) {
      filtro.estado = { $in: ['PENDIENTE', 'EVALUADO', 'AUTORIZADO', 'EN_EJECUCION'] };
    }
  } else if (esAdmin(rol)) {
  ...
  return ok(res, permisos, 200, { page, limit, total, totalPages: Math.ceil(total / limit) });
});
...
exports.crear = asyncHandler(async (req, res) => {
  if (req.user.rol !== 'SOLICITANTE' && !esAdmin(req.user.rol)) {
    throw new AppError('Solo el solicitante o un administrador puede abrir un PTRI', 403);
  }

  const {
    edificio, cota, zona_fuego, periodo_validez,
    tipos_trabajo, desplazamiento_multiple,
    descripcion_trabajo, responsable_solicitante, zona_controlada,
    herramientas,
  } = req.body;
  ...
  const permiso = await PermisoOperativo.create({
    ...
    descripcion_trabajo, responsable_solicitante,
    zona_controlada: !!zona_controlada,
    herramientas: herramientas || [],
    solicitante: req.user.id,
    estado: 'PENDIENTE',
    historico: [{ estadoAnterior: null, estadoNuevo: 'PENDIENTE', usuario: req.user.id, motivo: 'PTRI abierto' }],
  });
  await permiso.populate(POPULATE_BASE);

  tokensPorRol('BOMBERO', 'ADMIN').then((tokens) =>
    enviarNotificacion({
      tokens,
...
exports.evaluar = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior: 'PENDIENTE', estadoNuevo: 'EVALUADO', usuario: req.user.id, motivo: 'Evaluación PCI completada' });

  await permiso.save();
  await permiso.populate(POPULATE_BASE);

  tokensPorRol('JEFE', 'ADMIN').then((tokens) =>
    enviarNotificacion({
      tokens,
...
exports.autorizar = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior: 'EVALUADO', estadoNuevo: 'AUTORIZADO', usuario: req.user.id, motivo: req.body.motivo || 'Autorizado' });
...
exports.rechazar = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior: 'EVALUADO', estadoNuevo: 'RECHAZADO', usuario: req.user.id, motivo: req.body.motivo || '' });

  await permiso.save();
  await permiso.populate(POPULATE_BASE);

  tokenDeUsuario(permiso.solicitante).then((token) =>
    token && enviarNotificacion({
      tokens: [token],
...
exports.implantar = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior: 'AUTORIZADO', estadoNuevo: 'EN_EJECUCION', usuario: req.user.id, motivo: 'Trabajo implantado' });
...
exports.cerrar = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior: 'EN_EJECUCION', estadoNuevo: 'COMPLETADO', usuario: req.user.id, motivo: 'PTRI cerrado' });
...
exports.eliminar = asyncHandler(async (req, res) => {
  ...
  if (!estadosEliminables.includes(permiso.estado)) {
    throw new AppError('Solo se pueden eliminar permisos completados, rechazados o anulados', 400);
  }
...
exports.anular = asyncHandler(async (req, res) => {
  ...
  permiso.historico.push({ estadoAnterior, estadoNuevo: 'ANULADO', usuario: req.user.id, motivo: req.body.motivo || 'Anulado' });
